...
繁體中文遵循台灣軟體工程與本地化日常習慣，與簡體中文對應如下：

| 實體 / 概念 | en | zh-Hans | zh-Hant (繁體中文) |
| --- | --- | --- | --- |
| 任務 | Issue | 任务 | **任務** |
| 執行 | Task | task | **task** |
| 工作區 | Workspace | 工作区 | **工作區** |
| 代理人 / 智能體 | Agent | 智能体 | **代理人** |
| 專案 | Project | 项目 | **專案** |
| 執行環境 | Runtime | 运行时 | **執行環境** |
| 常駐程式 | Daemon | 守护进程 | **常駐程式** |
| 設定 | Settings | 设置 | **設定** |
| 預設 | Default | 默认 | **預設** |
| 儲存 | Save | 保存 | **儲存** |
| 使用者 | User | 用户 | **使用者** |
| 帳號 | Account | 账户 / 账号 | **帳號** |
| 收件匣 | Inbox | 收件箱 | **收件匣** |
| 資料庫 | Database | 数据库 | **資料庫** |
| 快取 | Cache | 缓存 | **快取** |
| 記憶體 | Memory | 内存 | **記憶體** |
| 終端機 | Terminal | 终端 | **終端機** |
| 外掛 / 擴充 | Plugin | 插件 | **外掛** |
| 意見回饋 | Feedback | 反馈 | **意見回饋** |
